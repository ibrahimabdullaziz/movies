...
const MovieCard = memo(({ movie }) => {
  ...
  return (
    <div
      ...
      className="group relative bg-surface rounded-xl overflow-hidden hover:ring-2 hover:ring-imdb-gold/50 transition-all duration-500 cursor-pointer shadow-lg hover:shadow-imdb-gold/20 aspect-[2/3]"
    >
      <div className="relative w-full h-full">
        <Poster path={movie.poster_path} title={movie.title} size="w300" isCard />

        <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-all duration-300 backdrop-blur-[3px] p-4 flex flex-col justify-end">
          <div className="mb-3 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 delay-75">
            <h2 className="text-sm font-bold text-white mb-1 line-clamp-1">{movie.title}</h2>
